...
export class Splitter extends React.Component<SplitterProps, SplitterState> {
    ...
    handleMouseMove = (e: any) => {
        ...
        const { handleBarOffsetFromParent, maxMousePosition } = this.state;
        ...
        const primaryPanePosition = getPrimaryPaneWidth(
            position,
            clientX,
            clientY,
            maxMousePosition,
    ...
    handleMouseUp = (e: any) => {
        ...
        const { handleBarOffsetFromParent, lastX, lastY, maxMousePosition } =
            this.state;
    ...
    render() {
        ...
        const { handleBarClonePosition, primaryPane, isVisible } = this.state;
    ...
    getSize = (cX?: Number | any, cY?: Number | any) => {
        ...
        * This function calculates the max position of a mouse in the current splitter from given percentage.
        ...
        let primaryPane = this.panePrimary
            .getDivInstance()
            .getBoundingClientRect();
        let handleBarSize = this.handlebar
            .getDivInstance()
            .getBoundingClientRect();

        const posInHandleBar =
            this.props.position === "vertical"
        ...
        if (this.props.position === "vertical") {
            ...
            let maxWidthStr = this.props.primaryPaneMaxWidth
                .match(regEx)[0]
                .toLowerCase();
            let maxWidthNum = parseFloat(
                this.props.primaryPaneMaxWidth.split(regEx)[0]
            );
            nodeWrapperSize = wrapper.width;
            primaryPaneOffset = primaryPane.left;

            if (maxWidthStr === "%") {
                maxMousePosition = Math.floor(
                    nodeWrapperSize * (maxWidthNum / 100) +
                        primaryPaneOffset -
                        (handleBarSize.width + posInHandleBar)
                );
            } else if (maxWidthStr === "px") {
                maxMousePosition = Math.floor(
                    maxWidthNum + primaryPaneOffset - handleBarSize.width
                );
            }
        } else {
            let maxHeightStr = this.props.primaryPaneMaxHeight
                .match(regEx)[0]
                .toLowerCase();
            let maxHeightNum = parseFloat(
                this.props.primaryPaneMaxHeight.split(regEx)[0]
            );
            ...
            if (maxHeightStr === "%") {
                maxMousePosition = Math.floor(
                    nodeWrapperSize * (maxHeightNum / 100) +
                        primaryPaneOffset -
                        (handleBarSize.height + posInHandleBar)
                );
            } else if (maxHeightStr === "px") {
                maxMousePosition = Math.floor(
                    maxHeightNum + primaryPaneOffset - handleBarSize.height
                );
            }
        }
